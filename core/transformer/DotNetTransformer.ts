...
//TODO: finish + testing
export class DotNetTransformer implements Transformer {
    transformArray(array: LSArray): string {
        return undefined;
    }
    transformComment(comment: string) {
        return new AndroidTransformer().transformComment(comment);
    }

    transformKeyValue(key: string, value: string) {
        //TODO: normalize string + detect format (%s => {0})

        return '<data name="' + key + '" xml:space="preserve">' + EOL
